...
def get_evolvable_mlp_config(
    ...
    default_hidden_size: Optional[list[int]] = None,
    ...
    if default_output_dim is None:
        default_output_dim = (
            default_hidden_size[-1] if default_hidden_size else 128
        )  # Default to last hidden or a fixed value
...
def get_evolvable_cnn_config(
    user_config: Optional[Dict] = None,
    default_fc_layer_dims: Optional[list[int]] = None,
) -> Dict:
...
def get_evolvable_lstm_config(
    user_config: Optional[Dict] = None, default_hidden_state_size: int = 256
) -> Dict:
    config = {
        "arch": "lstm",
    ...
    def __init__(
        ...
        if self.arch == "cnn":
            if not isinstance(observation_space, spaces.Box) or len(
                ...
                raise ValueError(
                    "CNN encoder requires Box observation space with 3 (e.g. C,H,W or H,W,C) or 4 (e.g. N,C,H,W or N,H,W,C) dims."
                )
            ...
            if len(obs_shape) == 3:  # (C,H,W) or (H,W,C)
                ...
                if obs_shape[0] in [1, 3, 4]:
                    input_channel, H, W = obs_shape[0], obs_shape[1], obs_shape[2]
                elif obs_shape[2] in [1, 3, 4]:
                    H, W, input_channel = obs_shape[0], obs_shape[1], obs_shape[2]
                else:
                    raise ValueError(
                        f"Unsupported image shape: {obs_shape}. Can't infer C,H,W ordering."
                    )
                self.input_shape = (input_channel, H, W)
            elif len(obs_shape) == 4:  # Assuming (N,C,H,W) or (N,H,W,C)
                if obs_shape[1] in [1, 3, 4]:
                    input_channel, H, W = obs_shape[1], obs_shape[2], obs_shape[3]
                elif obs_shape[3] in [1, 3, 4]:
                    H, W, input_channel = obs_shape[1], obs_shape[2], obs_shape[3]
                else:
                    raise ValueError(f"Unsupported batched image shape: {obs_shape}.")
                self.input_shape = (
                    input_channel,
                    H,
                    W,
                )  # EvolvableCNN takes single image shape
            else:
                raise ValueError(
                    f"Unsupported observation space shape for CNN: {obs_shape}"
                )
            ...
            self.base_encoder = EvolvableCNN(
                ...
                output_activation=cnn_config[
                    "output_activation"
                ],  # Usually None for features
            ...
            mlp_default_output_dim = (
                mlp_config_user.get("hidden_size")[-1]
                if mlp_config_user.get("hidden_size")
                else 128
            )
            mlp_config = get_evolvable_mlp_config(
                mlp_config_user, default_output_dim=mlp_default_output_dim
            )
            ...
            raise ValueError(
                f"Unsupported encoder architecture: {self.arch}. Choose 'cnn' or 'mlp'."
            )
        ...
        if self.is_recurrent:
            ...
            lstm_num_outputs = lstm_config["hidden_state_size"] * (
                2 if lstm_config.get("bidirectional") else 1
            )
...
class ICMInverseModel(EvolvableMLP):
    def __init__(
        ...
        resolved_config = get_evolvable_mlp_config(
            net_config,
            default_hidden_size=default_hidden,
            default_output_dim=action_dim,
        )

        # For continuous actions, we might want a different output activation
        output_activation = (
            None if self.is_continuous else None
        )  # Logits for discrete, raw for continuous
...
class ICMForwardModel(EvolvableMLP):
    def __init__(
        ...
        resolved_config = get_evolvable_mlp_config(
            net_config,
            default_hidden_size=default_hidden,
            default_output_dim=feature_dim,
        )
...
class ICM(EvolvableModule):
    def __init__(
        ...
        self.intrinsic_reward_weight = self.hps.get(
        ...
        if not isinstance(
            ...
            raise ValueError(
                "This ICM implementation currently supports Discrete, MultiDiscrete, and Box action spaces only."
            )
        ...
        if self.use_internal_encoder:
            self.encoder = ICMFeatureEncoder(
                observation_space, encoder_net_config, self.device
            )
            self.is_recurrent = (
                self.encoder.is_recurrent
            )  # Get from internally created encoder
            ...
            self.is_recurrent = (
                encoder_net_config.get("recurrent", False)
                if encoder_net_config
                else False
            )
        ...
        if feature_dim is not None:
            ...
            if self.use_internal_encoder:
                ...
                raise ValueError(
                    "`feature_dim` must be provided when `use_internal_encoder` is False."
                )

        self.inverse_model = ICMInverseModel(
            actual_feature_dim, action_space, inverse_net_config, self.device
        )
        self.forward_model = ICMForwardModel(
            actual_feature_dim, action_space, forward_net_config, self.device
        )
    ...
    def embed_obs(
        ...
        if not self.use_internal_encoder:
            if embedded_obs is None or embedded_next_obs is None:
                raise ValueError(
                    "embedded_obs and embedded_next_obs must be provided when using an external encoder."
                )
            ...
            if obs_batch is None or next_obs_batch is None:
                raise ValueError(
                    "obs_batch and next_obs_batch must be provided if using internal encoder."
                )
            if self.encoder is None:
                raise RuntimeError(
                    "ICM is configured to use an internal encoder, but it has not been initialized."
                )
        ...
        return (
            ...
            next_hidden_state_t if self.is_recurrent else None,
        )
    ...
    def compute_loss(
        ...
        (
            phi_obs,
            phi_next_obs,
            hidden_state_after_encoder_obs,
            hidden_state_after_encoder_next_obs,
        ) = self.embed_obs(
            obs_batch_t,
            next_obs_batch_t,
            embedded_obs,
